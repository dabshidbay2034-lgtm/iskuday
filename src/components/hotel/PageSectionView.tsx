...
/**
 * Menu items grouped by course, in the order the owner entered them.
 *
 * Deliberately NOT alphabetical: "Starters, Mains, Desserts" is the order a menu
 * is read in and sorting would put Desserts first. First appearance of each
 * category wins, which means the owner controls the running order simply by the
 * order they add dishes.
 *
 * Uncategorised items collect into one trailing, unlabelled group so a blank
 * category never hides a dish. If NOTHING is categorised the single group has no
 * heading, which renders exactly as the flat grid did before.
 */
function groupMenuItems(
  items: MenuItem[],
): { key: string; label: string | null; items: MenuItem[] }[] {
  const groups = new Map<string, { key: string; label: string | null; items: MenuItem[] }>();
  for (const item of items) {
    const label = item.category?.trim() || null;
    // Case-insensitive key so "Drinks" and "drinks" don't split into two
    // sections; the first spelling entered is the one displayed.
    // The leading space is load-bearing, not a typo: every real category is
    // trimmed before it gets here, so no owner-typed label can ever start with
    // one. That makes " uncategorised" a sentinel key that cannot collide with
    // a genuine category literally named "uncategorised".
    const key = label ? label.toLowerCase() : " uncategorised";
    const existing = groups.get(key);
    if (existing) existing.items.push(item);
    else groups.set(key, { key, label, items: [item] });
  }
  const all = [...groups.values()];
  // Uncategorised last, whatever order it was encountered in.
  return [
    ...all.filter((g) => g.label !== null),
    ...all.filter((g) => g.label === null),
  ];
}
...
export function PageSectionView({
  ...
  switch (section.type) {
    ...
    // A hotel policy clause. Reads like a text block with a label, so the
    // prebuilt policy page inherits the whole editor without new machinery.
    case "policy":
      if (!isEdit && !section.heading && !section.body) return null;
      return (
        <section
          className={cn(
            ...
            pick(section.align, (a) => alignClass(a), "text-left"),
          )}
        >
          {(isEdit || section.heading) && (
            <EditableText
              mode={mode}
              as="h3"
              className="font-heading font-bold text-base md:text-lg text-foreground mb-2"
              value={section.heading ?? ""}
      ...
      return (
        ...
        >
          ...
          {items.length === 0 ? (
            ...
            /* Grouped by course, the way a menu is actually read. The editor has
               always collected a category per dish; until now it was printed as
               a grey line under the name, which is data entry with no payoff.
               Items with no category fall into one unlabelled group so nothing
               is ever hidden by leaving the field blank. */
            <div className="space-y-8">
              {groupMenuItems(items).map((group) => (
                <div key={group.key}>
                  {group.label && (
                    <h3 className="font-heading font-semibold text-base text-foreground mb-3 pb-1.5 border-b border-border">
                      {group.label}
                    </h3>
                  )}
                  <div className={pick(section.columns, roomColumnsClass, "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4")}>
